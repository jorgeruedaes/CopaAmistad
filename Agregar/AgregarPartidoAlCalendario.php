...
<br><br>
  <link rel="stylesheet" href="formoid4_files/formoid1/formoid-flat-black.css" type="text/css" />
<script type="text/javascript" src="formoid4_files/formoid1/jquery.min.js"></script>
<form class="formoid-flat-black" style="background-color:#FFFFFF;font-size:16px;font-family:'Lato', sans-serif;color:#666666;max-width:480px;min-width:150px" method="get">
 ...
 <div class="element-date"><label class="title">Hora del partido:</label><input class="medium" type="time" name="hora" value="12:00:00" min="07:00:00" max="22:00:00" step="1"required/></div>
  ...
  <div ><label >Lugar:</label><div ><span><select name="lugar" required>
   ...
   <?php
$lugares=mysql_query("select id_lugar,nombre from tb_lugares");
while($lug=mysql_fetch_array($lugares)){
             ?>
...
?></select><i></i></span></div></div>
  <div ><label>Equipo Local:</label><div ><span><select name="EquipoA" required>
    ...
    <?php
   $EAs=mysql_query("SELECT id_equipo,nombre_equipo from tb_equipos ORDER BY nombre_equipo asc ");
while($EA=mysql_fetch_array($EAs)){
             ?>
...
?>
  ...
  <div ><label >Equipo Visitante:</label><div ><span><select name="EquipoB" required >
    ...
    <?php
   $EBs=mysql_query("SELECT id_equipo,nombre_equipo from tb_equipos ORDER BY nombre_equipo asc");
while($EB=mysql_fetch_array($EBs)){
             ?>
...
?></select><i></i></span></div></div>
  <div ><label >Juez:</label><div ><span><select name="juez" required>
    ...
    <?php
$lugares=mysql_query("select id_arbitro,nombre from tb_arbitros");
while($lug=mysql_fetch_array($lugares)){
             ?>
...
<script type="text/javascript" src="formoid4_files/formoid1/formoid-flat-black.js"></script>
<!-- Stop Formoid form-->
...
if (isset($_GET['cargar'])) {
  if ($_GET['EquipoA']==$_GET['EquipoB']) {
    ...
    $pal8=$_GET['NumeroF'];
    $pal9=$_GET['juez'];
 $mymodifi=mysql_query("INSERT INTO `tb_partidos`(`id_partido`, `equipo1`, `equipo2`, `resultado1`, `resultado2`, `fecha`, `numero_fecha`, `Lugar`, `Estado`, `Juez`,`hora`) VALUES (NULL,'$pal','$pal1',0,0,'$pal3','$pal8','$pal5','1','$pal9','$pal4');")or die(mysql_error());
